function [line]= ScrewTrajectory(Xstart, Xend, Tf, N, method)
...
%         0         0         0    1.0000
 ...
 a_r_1= a_r_1';
 a_r_1 = a_r_1(:);
 line(i,:) = [a_r_1', a_p_1',1];
 
 hold(:,:,i) =  Xstart * MatrixExp6(MatrixLog6(TransInv(Xstart) * Xend) * s);
%hold = [hold; Xstart * MatrixExp6(MatrixLog6(TransInv(Xstart) * Xend) * s)] ;
    traj{i} = Xstart * MatrixExp6(MatrixLog6(TransInv(Xstart) * Xend) * s);
end

%csvwrite('file1.csv',hold)
csvwrite('file1.csv',line)
end
